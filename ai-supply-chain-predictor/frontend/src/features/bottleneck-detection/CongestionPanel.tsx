...
const SEVERITY_COLORS: Record<string, { bg: string; border: string; text: string; bar: string }> = {
  Low:      { bg: 'bg-emerald-50 dark:bg-emerald-500/15',  border: 'border-emerald-200 dark:border-emerald-500/30',  text: 'text-emerald-600 dark:text-emerald-400',  bar: 'bg-emerald-500'  },
  Medium:   { bg: 'bg-amber-50 dark:bg-amber-500/15', border: 'border-amber-200 dark:border-amber-500/30', text: 'text-amber-600 dark:text-amber-400', bar: 'bg-amber-500' },
  High:     { bg: 'bg-orange-50 dark:bg-orange-500/15',  border: 'border-orange-200 dark:border-orange-500/30',  text: 'text-orange-600 dark:text-orange-400',  bar: 'bg-orange-500'  },
  Critical: { bg: 'bg-red-50 dark:bg-red-500/15',    border: 'border-red-200 dark:border-red-500/30',    text: 'text-red-600 dark:text-red-400',    bar: 'bg-red-500'    },
};
  ...
  return (
    <div>
      <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400 mb-1">
        <span>{label}</span>
        <span className="text-slate-900 dark:text-slate-100 font-mono font-semibold">{value}{unit}</span>
      </div>
...
export default function CongestionPanel() {
  ...
  return (
    <div className="glass-panel h-full flex flex-col">
      ...
      {refreshReason && (
        <div className="mb-3 rounded-lg border border-blue-200 dark:border-blue-500/30 bg-blue-50 dark:bg-blue-500/15 p-2 text-xs text-slate-700 dark:text-slate-300">
          {refreshReason}
        </div>
      )}

      {error && (
        <div className="mb-4 rounded-lg border border-red-500/20 bg-red-500/10 p-3 text-sm text-red-600 dark:text-red-300">
          {error}
        </div>
      )}
      ...
      {!bottleneck ? (
        <button
          onClick={() => detect()}
          disabled={loading}
          className="w-full mt-auto bg-slate-50 dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 px-4 py-3 rounded-lg font-medium transition-colors flex items-center justify-center gap-2"
        >
        ...
        <div className="space-y-3">
          ...
          <div className={`flex items-center gap-3 ${colors.bg} border ${colors.border} p-4 rounded-lg`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-xs mt-0.5 truncate">{bottleneck.cause}</p>
            </div>
          ...
          {/* Metric Bars */}
          <div className="bg-white dark:bg-slate-700/50 p-3 rounded-lg border border-slate-200 dark:border-slate-600 shadow-sm space-y-2.5">
            <MetricBar
          ...
          <div className={`grid grid-cols-2 gap-2 transition-all ${isRefreshing ? 'animate-pulse' : ''}`}>
            {bottleneck.bottleneckEdge && (
              <div className="bg-white dark:bg-slate-700/50 p-3 rounded-lg border border-slate-200 dark:border-slate-600 shadow-sm">
                ...
                <p className="font-mono text-sm text-slate-900 dark:text-slate-100 font-bold flex items-center gap-1">
            ...
            <div className="bg-white dark:bg-slate-700/50 p-3 rounded-lg border border-slate-200 dark:border-slate-600 shadow-sm">
              ...
              <div className="flex flex-wrap gap-1">
                {bottleneck.affectedNodes.map((n: string) => (
                  <span key={n} className="px-1.5 py-0.5 bg-slate-50 dark:bg-slate-600 text-slate-700 dark:text-slate-300 text-xs rounded font-mono border border-slate-200 dark:border-slate-500">{n}</span>
                ))}
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center gap-1 flex-wrap">
                {bottleneck.criticalPath.map((n: string, i: number) => (
                  <span key={i} className="flex items-center gap-1">
                    <span className="px-2 py-0.5 bg-slate-50 dark:bg-slate-600 text-slate-700 dark:text-slate-300 text-xs rounded font-mono border border-slate-200 dark:border-slate-500">{n}</span>
          ...
          {bottleneck.recommendation && (
            <div className="bg-blue-50/50 dark:bg-blue-500/10 p-3 rounded-lg border border-blue-100 dark:border-blue-500/30">
              <p className="text-xs text-blue-600 dark:text-blue-400 font-semibold mb-1">Recommended Action</p>
              <p className="text-xs text-slate-700 dark:text-slate-300">{bottleneck.recommendation}</p>
            </div>
          )}
          ...
          <div className="grid gap-2 sm:grid-cols-2 mt-auto pt-2">
            <button
              onClick={() => detect()}
              disabled={loading}
              className="w-full bg-slate-50 dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2 shadow-sm"
            >
              {loading ? <Activity className="animate-spin text-blue-500" size={14} /> : '↺ Re-scan'}
            </button>
            <button
              onClick={() => setLiveMode((value) => !value)}
              className={`w-full ${liveMode ? 'bg-amber-100 dark:bg-amber-500/20 hover:bg-amber-200 dark:hover:bg-amber-500/30 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-500/30' : 'bg-blue-600 hover:bg-blue-700 text-white border-blue-600'} border px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2 shadow-sm`}
            >
