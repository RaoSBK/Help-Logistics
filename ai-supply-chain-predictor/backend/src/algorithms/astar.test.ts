import { createMockNetwork } from './graph';
...
const runTests = () => {
  ...
  // Test 1: Normal Route
  const graph1 = createMockNetwork();
  const res1 = runAStar(graph1, 'A', 'D');
  ...
  const graph2 = createMockNetwork();
  graph2.applyDisruption('A', 'B', Infinity);
  const res2 = runAStar(graph2, 'A', 'D');
